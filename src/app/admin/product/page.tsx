...
const ProductPage = () => {
    ...
    const handleDeleteWithReload = async (id: string) => {
        if (confirm('Are you sure you want to delete?')) {
            await axios
                .delete(API_V1_URL + "/admin/product/" + id, {
                    headers: {
    ...
    return (
        <main className="container mx-auto mt-12">
            <section className="m-5">
                <h2 className="text-3xl mb-5 font-semibold">Product</h2>
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-md mb-5">
                    <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
                        ...
                        <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                            {error || isLoading
                                ? <RowBeforeLoad colSpan={7} error={error} isLoading={isLoading} />
                                : <TableRows items={products} perPage={perPage} currentPage={currentPage} url={route} onDelete={handleDeleteWithReload} />
                            }
                        </tbody>
                    </table>
                </div>
...
const TableRows = (
    ...
    if (items.length == 0) {
        return <tr className="my-3"><td colSpan={7} className="text-center">No data</td></tr>
    }
    ...
    return (<>
        {items?.map((item: any, key: any) =>
            <tr className="hover:bg-gray-50" key={item.id}>
                ...
                <td className="px-6 py-4">{item.discounted_price ?? item.price}</td>
                <td className="px-6 py-4 font-semibold">{item.stock}</td>
                <td className="px-6 py-4">
                    <span className="font-medium">{item.supplier.code}</span>
                    <br />{item.supplier.name}
                </td>
                <td className="px-6 py-4">
